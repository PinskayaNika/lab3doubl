...
//        JavaPairRDD<Pair<Integer, Integer>, String> flightStringConverted = dataToString(flightReduced);
//        return flightStringConverted;
//    }
//
...
public class FlightFunctions {
    ...
    private static JavaPairRDD<Pair<Integer, Integer>, float[]> createSchedulePair(JavaRDD<String> schedule){
        return schedule.mapToPair(s -> {
            Pair<Integer, Integer> airportsIDs = new Pair<>(Integer.parseInt(AirportFunctions.getAirportData(AIRPORT_ID_FROM_POS,s,false)),Integer.parseInt(AirportFunctions.getAirportData(AIRPORT_ID_TO_POS,s,false)));
            float[] delayData = new float[]{0,0,0,0,1};
            if (AirportFunctions.getAirportData(DELAY_FLIGHT_POS, s, false).length() > 0) {
                delayData[MAX_DELAY_POS] = Float.parseFloat(AirportFunctions.getAirportData(DELAY_FLIGHT_POS,s,false));
                delayData[NUMBER_OF_DELAYED_POS] = 1;
